...
from constants import STYLE_INDEX, Q1_IMAGE
...
class Segmentor:
    ...
    def cluster(self, res: int = 32):
        ...
        struct_attn = self_attn[Q1_IMAGE].mean(dim=0).cpu().numpy()
    ...
    def get_object_masks(self) -> Tuple[torch.Tensor]:
        ...
        mask_struct_32 = self.create_mask(clusters_struct_32, self.cross_attention_32, Q1_INDEX)
        mask_style_64 = self.create_mask(clusters_style_64, self.cross_attention_64, STYLE_INDEX)
        mask_struct_64 = self.create_mask(clusters_struct_64, self.cross_attention_64, Q1_INDEX)
